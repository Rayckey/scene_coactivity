...
def gather_positionnet(room_type, GUI, gen_images, gen_graph, collect):
    test_rooms = []
    ...
    for cat in suncg_object_types:
        positionnet.add_node(cat.replace("_", " "))

    for cat1 in positionnet:
        for cat2 in positionnet:
            positionnet.add_edge(cat1, cat2, NEXTTO=[], CLOSETO=[], CLOSETOBOUND=[],
        ...
        if collect:
            ...
            for id1 in pos:
                for id2 in pos:
                    if id1 == id2:
                        continue
                    # positionnet[pos.nodes()[id1]['cat'].replace("_", " ")][pos.nodes()[
    ...
    pickle.dump(positionnet, fi)
    print('Finished! ')


def fit_positionnet(room_type):
...
if __name__ == '__main__':
    ...
    if args.collect or args.images or args.graph:
        gather_positionnet(args.room, False, args.images,
