...
def convert_raw_to_room_info(room_data, model_bb_mapping, model_cat_mapping):
    furn_info = room2networkx(room_data, model_bb_mapping, model_cat_mapping)
...
# def viewSingleResult(scene_info: SceneInformation, opt_config, reach=False, app = False, path=False, color=False,
#         use_walls=True, GUI=False, snip_shot=False, name=''):
...
#     #         new_pose = R.from_euler('xyz', furn_info.nodes()[id]['pose'][1] ).as_matrix() @ np.array([0,0, -0.3]) + np.array(furn_info.nodes()[id]['pose'][0])
#     #         furn_info.nodes()[id]['pose'][0] = new_pose.tolist()
#     # ------------------------------------------

#     if not (reach or app or path or color):
#         visualizeFurnitureInfo(
#             furn_info, seg_map, grouping_order, use_walls, GUI, snip_shot, name)
#     else:
#         visualizeOverlayInfo(
#             furn_info, opt_config, task_config, robot, seg_map, grouping_order, use_walls, GUI, snip_shot, name, overlay)


# def viewOptimationResults(res_hist: ResultHistory, reach, app, path, color,  
#         GUI=False, snip_shot=True, prefix='', resolution=2000):
#     opt_config, task_config, stepping_config, scene_hist, hist = res_hist
#     idx = 0
#     for scene_info in scene_hist:
#         viewSingleResult(scene_info, opt_config, use_walls=False, reach= reach, app = app, path=path, color=color, 
#                          GUI=GUI, snip_shot=snip_shot, name=f"{prefix}_{idx}", resolution= resolution)
#         idx += 1

    # scene_info = scene_hist[-1]
    # viewSingleResult(scene_info, opt_config, use_walls=False, overlay=overlay,
    #                  GUI=GUI, snip_shot=snip_shot, name=f"{prefix}_{-1}")
...
def visualizeOverlayInfo(furn_info, opt_config: AdaptiveAnnealingConfig, task_config: TaskConfig, robot: dict, seg_map=None,
    ...
    for id in plot_ids:
        # ==================================================
        # if furn_info.nodes()[id]['cat'] == 'door':
        #     new_pose = R.from_euler('xyz', furn_info.nodes()[id]['pose'][1]).as_matrix(
        #     ) @ np.array([0, 0, 0.3]) + np.array(furn_info.nodes()[id]['pose'][0])
        #     furn_info.nodes()[id]['pose'][0] = new_pose.tolist()
        # if furn_info.nodes()[id]['cat'] == 'television':
        #     new_pose = R.from_euler('xyz', furn_info.nodes()[id]['pose'][1]).as_matrix(
        #     ) @ np.array([0, 1.0, 0.0]) + np.array(furn_info.nodes()[id]['pose'][0])
        #     furn_info.nodes()[id]['pose'][0] = new_pose.tolist()
        # ==================================================
