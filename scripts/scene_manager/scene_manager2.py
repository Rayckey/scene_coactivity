...
class SceneManager():
    ...
    def loadProcessedPositionNet(self, path: str):
        fi = open(path, "rb")
    ...
    def objectiveFunction(self, state=None):
        ...
        if 'interaction' in self.objectives:
            ...
            use_sdf = reachability_sdf
            # use_sdf = object_sdf
            for id in self.approch.keys():
                aff_total = self.approch_buffer[id]['best']
                # aff += np.sum(getCenteredSDF(
                #     self.approch[id], self.w, self.h) * use_sdf)
                aff = np.sum(getCenteredSDF(
                    self.approch[id], self.w, self.h) * use_sdf)
        ...
        if 'anti' in self.objectives:
            ...
            for id in self.oppose.keys():
                ant_total = self.opposite_buffer[id]['best']
                # ant += np.sum(self.oppose[id] * object_sdf_pad)
                ant = np.sum(self.oppose[id] * use_sdf)
        ...
        if 'relation' in self.objectives:
            ...
            rel_list = []
            # for id1 in self.plot_ids:
            #     for id2 in self.plot_ids:
            #         if id1 == id2 :
            #             continue
            # pairs = list(combinations(self.plot_ids, 2))
            for relation in self.relations:
                ...
                # NEXTTO = self.position_net[cat1][cat2]['NEXTTO'] * \
                #     satuating_cost(NEXTTO)
                # CLOSETO = self.position_net[cat1][cat2]['CLOSETO'] * \
                #     satuating_cost(CLOSETO) if CLOSETO > 0 else 0.
                # FRONTOF = self.position_net[cat1][cat2]['FRONTOF'] * ratio_cost(FRONTOF, np.pi)
                # SIDEOF = self.position_net[cat1][cat2]['SIDEOF'] * ratio_cost(SIDEOF, np.pi/2.)
                # BACKOF = self.position_net[cat1][cat2]['BACKOF'] * ratio_cost(BACKOF, np.pi)
                # FACING = self.position_net[cat1][cat2]['FACING'] * ratio_cost(FACING, np.pi)
                # AGAINST = self.position_net[cat1][cat2]['AGAINST'] * ratio_cost(AGAINST, np.pi)
                # AWAY = self.position_net[cat1][cat2]['AWAY'] * ratio_cost(AWAY, np.pi/2.)
                # PARALLEL = self.position_net[cat1][cat2]['PARALLEL'] * ratio_cost(PARALLEL, np.pi)
                # OPPOSE = self.position_net[cat1][cat2]['OPPOSE'] * ratio_cost(OPPOSE, np.pi)
                # ORTHOGONAL = self.position_net[cat1][cat2]['ORTHOGONAL'] * ratio_cost(ORTHOGONAL, np.pi/2.)
                ...
                # # SIDEOF = self.getSideofCost(idx_n=id2, idx_p=id1, val=FRONTOF)  # on +- x axis
                # # BACKOF = self.getBackofCost(idx_n=id2, idx_p=id1, val=FRONTOF)  # on -z axis
                # FACING = self.getFacingCost(idx_n=id2, idx_p=id1)  # id1 is facing id2
                # FACINGBOUND =  self.getFacingBoundCost(idx_n=id2, idx_p=id1)
                # # AGAINST = self.getAgainstCost(idx_n=id2, idx_p=id1, val=FACING)
                # # AWAY = self.getAwayCost(idx_n=id2, idx_p=id1, val=FACING)
                # PARALLEL = self.getParallelCost(idx_n=id2, idx_p=id1)# id1 facing same direction as id2
                # # OPPOSE = self.getOpposeCost(idx_n=id2, idx_p=id1, val=PARALLEL)
                # # ORTHOGONAL = self.getOrthogonalCost(idx_n=id2, idx_p=id1, val=PARALLEL) # id1 facing opposite direction as id2
                ...
                # rel_list.append(NEXTTO+CLOSETO+FRONTOF+SIDEOF +
                #                 BACKOF+FACING+AGAINST+AWAY+PARALLEL+OPPOSE+ORTHOGONAL)

                # rel_list.append(NEXTTO_cost+CLOSETO_cost+FRONTOF_cost+FACING_cost+PARALLEL_cost)

            # rel_cost = WEIGHTS['relation'] * np.sum(rel_list)/len(rel_list)/5.
            rel_cost = WEIGHTS['relation'] * np.sum(rel_list)
            cost += rel_cost

            # rel_cost = WEIGHTS['relation'] * \
            #     np.sum(rel_list)/len(rel_list) if len(rel_list) > 0 else 0
            # cost += rel_cost
    ...
    def evaluateReachability(self):
        ...
        total_aff = 0
        max_aff = 0
        for id in self.approch:
            temp_app = getCenteredSDF(self.approch[id], self.w, self.h)
            total_aff += np.sum((reachability_sdf > 0).astype('float') * (
                temp_app > hinge_loss(self.robot['reach'], self.robot['reach']).astype('float')))
            temp_max_app = self.approch_buffer[id]['pic']
    ...
    def findInteractPose(self, reachability_sdf, id, coords):
        if id not in self.approch.keys():
            return None, None

        weight = getCenteredSDF(self.approch[id], self.w, self.h)
    ...
    def createIndividualSDFBuffer(self, id):
        ...
        if self.furniture_info.nodes()[id]['cat'] in HIGHUP or self.furniture_info.nodes()[id]['cat'] in LOWERDOWN or self.furniture_info.nodes()[id]['cat'] == 'door':
            ...
            self.approch_buffer[id]['best'] = self.getBestCaseApprochability(
                id)
            self.opposite_buffer[id]['best'] = self.getBestCaseOpposition(id)
    ...
    def createIndividualApprochabilityBuffer(self, id, extent, coordinates):
        ...
        self.approch_buffer[id] = {'pic': pic,
                                   'extent': extent, 'coordinates': coordinates}
    ...
    def createIndividualOppositeBuffer(self, id, extent, coordinates):
        ...
        self.opposite_buffer[id] = {'pic': pic,
                                    'extent': extent, 'coordinates': coordinates}

    def getBestCaseApprochability(self, id):
        if self.approch_buffer[id]['pic'] is None:
            return 0.
        return np.sum(self.approch_buffer[id]['pic'] * self.sdf_buffer[id]['pic'])

    def getBestCaseOpposition(self, id):
        if self.opposite_buffer[id]['pic'] is None:
            return 0.
        return np.sum(- self.opposite_buffer[id]['pic'] * self.sdf_buffer[id]['pic'])
    ...
    def discritizeSDFfromBuffer(self, include_ids=[], use_approach=False, use_oppose=False):
        ...
        for id in include_ids:
            ...
            _approah = self.approch_buffer[id]['pic']
            _oppose = self.opposite_buffer[id]['pic']
            ...
            if use_approach and _approah is not None:
                self.approch[id] = np.zeros_like(img)
                self.approch[id][logic_idx[0], logic_idx[1]
                                 ] = _approah[valid_coords_in_furn_img[:, 0], valid_coords_in_furn_img[:, 1]]
                approach += self.approch[id]
            if use_oppose and _oppose is not None:
                self.oppose[id] = np.zeros_like(img)
                self.oppose[id][logic_idx[0], logic_idx[1]
                                ] = _oppose[valid_coords_in_furn_img[:, 0], valid_coords_in_furn_img[:, 1]]
                oppose += self.oppose[id]
    ...
    def getStartPose(self):
        ...
        #     # door_sem['front']) * door_sem['bb'][2]/2.
        # relative_pose = np.append(relative_pose, 1.)
        # door_pose1 = door_trans @ relative_pose
